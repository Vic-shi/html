...
function findAuthorMsg(pageNo){
	...
	pageno = pageNo+1;
	var url="/web/api/recommendAuthor/listAuthor";
	var listLen = 10;
	var authorName = document.getElementById("authorName").value.trim();
	var authorLevel = document.getElementById("authorLevel").value;
	var recommendStatus = document.getElementById("authorStatus").value;
	var param = {
			url: url,
			data: {"jsonStr":"{'pageNo':"+pageNo+",'listLen':"+listLen+",'draw':1,'authorName':'"+authorName+"','authorLevel':"+authorLevel+",'recommendStatus':"+recommendStatus+"}"},
...
tbody.append("<tr>"
		...
		+"<td class='center'><img src='"+item.authorPortrait+"' class='master_img head_img tr_height'></td>"
		+"<td class='center forced_wrap'>"+(item.authorLevel == 0 ? "国家级" : (item.authorLevel == 1 ? "中书协/中美协":(item.authorLevel == 2 ? "省级":"未知")))+"</td>"
		+"<td class='center'>"+start+"到"+end+"</td>"
		+"<td class='center'>"+(item.recommendStatus == 1 ? "往期推荐" : item.recommendStatus == 0 ? "推荐中" : item.recommendStatus == 2 ? "已删除" : "未知")+"</td>"
...
//展示推荐艺术家内容（先用查询接口代替）
function findAuthorHead(){
	document.getElementById("tbody_head").innerHTML="";
	document.getElementById("paging_head").innerHTML=""; 
	var url="/web/api/recommendAuthor/listAuthor";
	var listLen = 10;
	var pageNo = 1;//真实对接中删除
	var authorName = document.getElementById("authorName").value.trim();//真实对接中删除
	var authorLevel = document.getElementById("authorLevel").value;//真实对接中删除
	var authorStatus = document.getElementById("authorStatus").value;//真实对接中删除
	var param = {
			url: url,
			data: {"jsonStr":"{'pageNo':"+pageNo+",'listLen':"+listLen+",'draw':1,'authorName':'"+authorName+"','authorLevel':"+authorLevel+",'recommendStatus':0}"},
			successCallback: function(data){
				var tbody = $("#tbody_head");
				var error = $('#error_head');
				var paging = $("#paging_head");
				var listAuthor = data.data.listAuthor;
				if(data.data.page.totalCount==0){
					var str="<span>查无数据<span>";
					error.html(str);
...
//传入从后台取回的参数
function OutPutHead(tbody, item) {
	if(item.recommendStatus == 0){
		var startTime = new Date(item.startTime);
	    var startYear = startTime.getFullYear();
	    var startMonth = startTime.getMonth()+1;
		...
		tbody.append("<tr>"
			+"<td class='center forced_wrap'>"+item.authorId+"</td>"
			+"<td class='center forced_wrap'>"+item.authorName+"</td>"
			+"<td class='center'><img src='"+item.authorPortrait+"' class='head_img'></td>"
			+"<td class='center forced_wrap'>"+(item.authorLevel == 0 ? "国家级" : (item.authorLevel == 1 ? "中书协/中美协":(item.authorLevel == 2 ? "省级":"未知")))+"</td>"
			+"<td class='center forced_wrap'>"+startYear+"-"+startMonth+"-"+startDate+"</td>"
			+"<td class='center'>"+(item.recommendStatus == 1 ? "往期推荐" : item.recommendStatus == 0 ? "推荐中" : item.recommendStatus == 2 ? "已删除" : "未知")+"</td>"
			+"<td class='center'><img src='"+item.authorPosters+"' class='master_img'></td>"
			+"<td class='center'><img src='/web/commons/image/management/replace_icon.png' onclick='chanceTo();' class='delect'></td>"
			+"</tr>");
	}
}
//跳转去艺术家列表选择
function chanceTo(){
	window.location.href="/web/management/erp/master/recommend_list.jsp";
}
...
function deleteAuthor(authorId){
	if($("#bomb_positioning").is(":hidden")){
		...
		$(".confirm_button").bind("click",function(){
			var param = {
					url: '/web/api/recommendAuthor/deleteBatch',
