...
function findCarousel(){
	//判断select内容，确定走什么接口
	var type=document.getElementById("carouselType");
	if(type.value == "product"){
		findProducts(1);
	}else if(type.value == "author"){
		findAuthorMsg(1);
	}else if(type.value == "information"){
...
function findInformationPage(pageNo,erp_news_Keywords) {
	...
	var informationParam={
			url:url,
			data:{"jsonStr":"{'pageNo':"+pageNo+",'title':'"+erp_news_Keywords+"','pageSize':"+pageSize+",'informationStatus':'"+informationStatus+"','carouselType':'"+carouselType+"'}"},
			successCallback: function(data) {
				...
				if(data.state == 0){
					...
					if(data.data.totalCount==0){
						...
						error.html("");
						var thead = '<tr><th class="advertisement_serial_number">编号</th><th class="advertisement_img">轮播主图</th>'+
			        		'<th class="advertisement_title">标题</th><th class="advertisement_time">创建时间</th><th class="advertisement_btn"></th></tr>';
						$('#thead').html(thead);
...
function outPutInformationContent(tbody, item, test) {
	...
	var id = "carousel"+item.id;
	var str = "<tr id='carousel"+item.id+"'>"
...
function verify(){
	var carouselType = $("#carouselType").val();
	var contentId = carouselObj.getElementsByTagName('td')[0].innerText;
	var carouselId = sessionStorage.getItem("carouselId");
	var imageAddress = carouselObj.getElementsByTagName('td')[1].getElementsByTagName('img')[0].src;
	var posterAddress = document.getElementById('img_upload').src;
	var title = document.getElementById('title').value;
	var url = "/web/api/bam/updateCarousel";
	var carousel = "{'carouselId':'"+carouselId+"','carouselType':'"+carouselType+"','contentId':'"+contentId+"','title':'"+title+"','imageAddress':'"+imageAddress+"','posterAddress':'"+posterAddress+"'}";
	var data = {"jsonStr":"{'carousel':"+carousel+"}"};
	var updateCarouselParam = {
		method: 'POST',
		url : url,
		data : data,
		successCallback: function(data) {
			if(data.state == 0){
				$(location).attr('href', '/web/management/platform/news/advertisement_list.jsp');
			}else{
				layer.msg('提交失败');
			}
		},
	...
	getDataFromServer(updateCarouselParam);
}
//输出资讯分页选择栏
...
function findActivityPage(pageNo) {
	...
	var activityParam={
			url:url,
			data:{"jsonStr":"{'pageNo':"+pageNo+",'title':'"+erp_news_Keywords+"','pageSize':"+pageSize+",'activityStatus':'"+activityStatus+"','carouselType':'"+carouselType+"'}"},
			successCallback:function(data) {
				console.log(data);
				if(data.state == 0){
					index = 0;
					...
					if(data.data.totalCount==0){
						...
						error.html("");
						var thead = '<tr><th class="advertisement_serial_number">编号</th><th class="advertisement_img">轮播主图</th><th class="advertisement_title">标题</th>'+
		        			'<th class="advertisement_time">活动开始时间</th><th class="advertisement_time">活动结束时间</th><th class="advertisement_btn"></th></tr>';
						$('#thead').html(thead);
						$.each(pageContent, function (index, item) {
							test = test + 1;
...
function findProducts(pageNo){
	...
	var productParam={
			url:url,
			data:{"jsonStr":"{'pageNo':"+pageNo+",'listLen':"+listLen+",'productMsg':'"+erp_news_Keywords+"'}"},
			successCallback:function(data){
				console.log(data);
				if(data.state == 0){
					index=0;
					var pageContent = data.data.listProduct;
					var totalPages = data.data.page.totalPages;
					var tbody = $("#tbody");
					var error = $('#error');
					var pageSelection = $('#page_selection');
					var test = (pageno-1)*5;
					var thead = '<tr><th class="advertisement_serial_number">编号</th><th class="advertisement_img">轮播主图</th><th class="advertisement_title">名称</th><th class="advertisement_title">作者</th>'+
						'<th class="advertisement_btn"></th></tr>';
					$('#thead').html(thead);
					if(data.data.page.totalCount==0){
						...
						$.each(pageContent, function (index, item) {
							test = test + 1;
							outPutProductData(tbody, item, test);
						});
						outPutPageProductSelection(pageSelection, pageContent, totalPages);
					}
				}else{
					getDataFromServer(productParam);
				}
			},
			errorCallBack:function(data) {
				layui.use('layer', function(){
...
//输出作品分页内容
function outPutProductData(tbody, item, test) {
	arrayIds[index] = item.id;
	console.log(item);
	var id = "carousel"+item.id;
	var str = "<tr id='carousel"+item.id+"'>"
			+"<td class='center title'>"+item.id+"</td>"
			+"<td class='center'><img class='tr_height' src='"+item.productPictureMain+"'></td>"
			+"<td class='center title'>"+item.productName+"</td>"
			+"<td class='center title'>"+item.productAuthor+"</td>"
			+"<td class='center'><input type='button' class='confirm_the_selection' value='确定选择' onclick='onPopup("+id+",1)'></td>"
		+"</tr>";
	tbody.append(str);
}//输出作品分页选择栏
function outPutPageProductSelection(pageSelection, pageContent, totalPages) {
	pageSelection.html("");
	var lastPage = pageno - 1;
	var nextPage = pageno + 1;
	var str = "<button id='first_page' class='last_page' onclick='findProducts("+1+")'>首页</button>"
	+ "<button id='prev_page' class='next_page' onclick='findProducts("+lastPage+")'>上一页</button>";
	pageSelection.append(str);
	for(var i=1; i<=totalPages; i++) {
		var str="";
		//当前页按钮禁用
		if(i==pageno){
			str = "<button class='pages' value='"+i+"'>"+i+"</button>";
			 ...
			 str = "<button onclick='findProducts("+i+");' class='page' value='"+i+"'>"+i+"</button>";
		}
		pageSelection.append(str);
	}
	var str = "<button onclick='findProducts("+nextPage+");' id='next_page' class='next_page' value='"+nextPage+"'>下一页</button>"
			+ "<button onclick='findProducts("+totalPages+");' id='last_page' class='last_page' value='"+totalPages+"'>末页</button>"
			+ "<input id='jumpPageNo' type='text' class='page_number_input_box' onkeyup='onlyNumber()'>"
			+ "<button class='pages_jump' onclick='jumpByPageNo_product("+totalPages+");'>跳转</button>";
	pageSelection.append(str);
	//判断上一页等按钮是否需要禁用
	pages(totalPages);
	/*//点击确认选择后把数据存入轮播表
	$(".confirm_the_selection").click(function(){
		...
		if(title.val()==""){
			...
			getDataFromServer(saveProductParam);
		}
	});*/
}
//跳转作品指定页
function jumpByPageNo_product(totalPages){
	var jumpPageNo = document.getElementById("jumpPageNo").value.trim();
	if(jumpPageNo < 1) {
		jumpPageNo = 1;
	} else if(jumpPageNo > totalPages) {
...
function findAuthorMsg(pageNo){
	...
	var masterParam={
			url:url,
			data:{"jsonStr":"{'pageNo':"+pageNo+",'listLen':"+listLen+",'authorName':'"+erp_news_Keywords+"'}"},
			successCallback:function(data){
				console.log(data);
				if(data.state == 0){
					index = 0;
					var pageContent = data.data.listAuthor;
					var totalPages = data.data.page.totalPages;
					var tbody = $("#tbody");
					var error=$("#error");
					var pageSelection = $('#page_selection');
					var test = (pageno-1)*5;
					var thead = '<tr><th class="advertisement_serial_number">编号</th><th class="advertisement_img">轮播主图</th>'+
						'<th class="advertisement_title">标题</th><th class="advertisement_btn"></th></tr>';
					$('#thead').html(thead);
					if(data.data.page.totalCount==0){
						var str="<span style='color:#fe0338;'>查无数据<span>";
						error.html(str);
						...
						$.each(pageContent, function (index, item) {
							test = test + 1;
							outPutAuthorData(tbody, item, test);
						});  
						outPutPageAuthorSelection(pageSelection, pageContent, totalPages);
					}
				}else{
					getDataFromServer(masterParam);
				}
			},
			errorCallBack:function(data) {
				layui.use('layer', function(){
...
//输出作者分页内容
function outPutAuthorData(tbody, item, test) {
	arrayIds[index] = item.id;
	var id = "carousel"+item.id;
	var str = "<tr id='carousel"+item.id+"'>"
			+"<td class='center title'>"+item.id+"</td>"
			+"<td class='center'><img class='tr_height' src='"+item.authorPicturePublicity+"'></td>"
			+"<td class='center' hidden='hidden'>"+test+"</td>"
			+"<td class='center title'>"+item.authorName+"</td>"
			+"<td class='center'><input type='button' class='confirm_the_selection' value='确定选择' onclick='onPopup("+id+",2)'></td>"
		+"</tr>";
	tbody.append(str);
}
...
//输出作者分页选择栏
function outPutPageAuthorSelection(pageSelection, pageContent, totalPages) {
	pageSelection.html("");
	var lastPage = pageno - 1;
	var nextPage = pageno + 1;
	var str = "<button id='first_page' class='last_page' onclick='findAuthorMsg("+1+")' >首页</button>"
	+ "<button id='prev_page' class='next_page' onclick='findAuthorMsg("+lastPage+")'>上一页</button>";
	pageSelection.append(str);
	for(var i=1; i<=totalPages; i++) {
		var str="";
		//当前页按钮禁用
		if(i==pageno){
			str = "<button class='pages' value='"+i+"'>"+i+"</button>";
			 ...
			 str = "<button onclick='findAuthorMsg("+i+");' class='page' value='"+i+"'>"+i+"</button>";
		}
		pageSelection.append(str);
	}
	var str = "<button onclick='findAuthorMsg("+nextPage+");' id='next_page' class='next_page' value='"+nextPage+"'>下一页</button>"
			+ "<button onclick='findAuthorMsg("+totalPages+");' id='last_page' class='last_page' value='"+totalPages+"'>末页</button>"
			+ "<input id='jumpPageNo' type='text' class='page_number_input_box' onkeyup='onlyNumber()'>"
			+ "<button class='pages_jump' onclick='jumpByPageNo_Author("+totalPages+");'>跳转</button>";
	pageSelection.append(str);
	//判断上一页等按钮是否需要禁用
	pages(totalPages);
	/*//点击确认选择后把数据存入轮播表
	$(".confirm_the_selection").click(function(){
		...
		if(title.val()==""){
			...
			getDataFromServer(saveMasterParam);
		}
	});*/
}

//跳转作者指定页
function jumpByPageNo_Author(totalPages){
	var jumpPageNo = document.getElementById("jumpPageNo").value.trim();
	if(jumpPageNo < 1) {
		jumpPageNo = 1;
	} else if(jumpPageNo > totalPages) {
