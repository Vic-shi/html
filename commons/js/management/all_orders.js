...
layui.use('form', function(){
  ...
  $("dd").removeClass("layui-this");
	//修改选择框里的值
	$("#select_orderStatus dl").each(function(){
		var dd=$(this).children().first();
		dd.text("全部订单状态");
		$("#select_orderStatus input").prop("placeholder", "选择订单状态");
	});
  //监听提交
...
function OutPutData(tbody, item) {
	...
	tbody.append("<tr>"
				...
				+"<td class='center product_author'>"+item.productAuthor+"</td>"
				+"<td class='center forced_wrap'>"+item.userName+"</td>"
				+"<td class='center forced_wrap'>"+item.userMobile+"</td>"
				+"<td class='center' >地址</td>"
				/*+"<td class='center' >"+getEndDate(item.orderCreateTime,item.termType)+"</td>"
				+"<td class='center' >"+(item.isOverdue == true ? "逾期" : "未逾期")+"</td>"
				+"<td class='center' >"+item.productMountedValue+"</td>"
				+"<td class='center' >"+item.productDeposit+"</td>"*/
