...
function outPutPageContent(tbody, item) {
	...
	if(item.activityCourse==0){
		var tatus="预约中";
	}else if(item.activityCourse==1){
		var tatus="进行中";
	}else if(item.activityCourse==2){
		var tatus="已结束";
	}
	var str = '<tr>'
				+'<td class="center">'
				...
				+'<td class="center"><img src="/web/commons/image/management/edit_icon.png" onclick="editActivity('+index+');" class="edit_icon"></td>'
...
function editActivity(i) {
	var activityId = arrActivityIds[i];
	sessionStorage.setItem("activityId",activityId);
	window.location.href='/web/management/platform/news/activity_edit.jsp';
}
