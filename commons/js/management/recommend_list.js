...
tbody.append("<tr>"
		...
		+"<td class='center'><img src='"+item.authorPicturePortrait+"' class='head_img tr_height'></td>"
		...
		+"<td class='center'><img src='"+item.authorPicturePublicity+"' class='master_img'></td>"
		+"<td class='center'><input type='button' class='confirm_the_selection' value='确定选择' onclick='addRecommendAuthor("+index+");'></td>"
		+"</tr>");
}
...
//单条老师信息
function addRecommendAuthor(index){
	var author = arrAuthor[index];
	console.log(author);
	var url="/web/api/recommendAuthor/save";
	var param = {
			url: url,
			data: {"jsonStr":"{'authorId':'"+author.authorId+"','authorName':'"+author.authorName+"','authorLevel':"+author.authorLevel+",'authorPosters':'"+author.authorPicturePublicity+"','authorPortrait':'"+author.authorPicturePortrait+"','inputStaffNo':112211,'recommendStatus':0,'isSignUp':'"+author.isSignUp+"'}"},
			successCallback: function(data){
				window.location.href = "/web/management/erp/master/recommend_master.jsp";
				console.log(data);
			},
			errorCallBack: function(data) {
				console.log(data.msg);
